...
class TaskManager {
	 ...
	 * @throws Exception
	 */
	function doTask($id, $params = [], $callback = null, $bindArgs = []){
		if(!isset($this->callbacks[$id])){
			throw(new Exception("Invalid task id"));
		}

		$taskData = $this->getTaskArgs([$id, $params]);
		if($this->server->taskworker){
			// 在task进程, 任务同步执行
			$rs = $this->processTask($taskData);
			if($callback){
				call_user_func($callback, $rs, $bindArgs);
			}
		} else {
			$this->server->task($taskData, -1, function($serv, $task_id, $data) use($callback, $bindArgs) {
				if($callback){
					call_user_func($callback, $data, $bindArgs);
				}
			});
		}
	}
	...
	public function doTasks($requests, $callback = null, $bindParams = [], $timeout = 10.0){
		if($this->server->taskworker){
            ...
            $counter = 0;
			foreach($requests as $index => $request){
				$tasks[] = $this->getTaskArgs($request);
                $indexMap[$counter++] = $index;
			}
			...
			// 索引被重置为数字
			$tmp = [];
            foreach($indexMap as $index => $key){
                $tmp[$key] = $results[$index];
            }
